/**
 * @file VoiceRecordButton.test.jsx
...
describe('components', () => {
  describe('VoiceRecordButton', () => {
    it('should mount with props', function () {
      const wrap = mount(<VoiceRecordButton>Hello World</VoiceRecordButton>)
    ...
    it('should render as type and with content', function () {
      const wrap = render(<VoiceRecordButton>Hello World</VoiceRecordButton>)

      expect(wrap[0].type).toEqual('tag')
      expect(wrap[0].name).toEqual('div')
      // contain styles
      expect(wrap[0].attribs.class).toContain('voice_record_button')
